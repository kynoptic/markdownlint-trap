...
/**
 * Test suite for autofix functionality of the backtick-code-elements rule.
 * Ensures that the autofix produces the expected output for a fixture with code element violations.
 */
describe("backtick-code-elements auto-fix functionality", () => {
  ...
  /**
   * Verifies that the autofix output matches the expected fixed content.
   */
  test("applies auto-fixes and matches expected output", async () => {
    const options = {
    ...
    const ruleFixes = (results[tempFilePath] || []).filter(
      (v) =>
  ...
  test("applies auto-fixes for minimal single-violation-per-line fixture", async () => {
    ...
    const minimalTempFilePath = path.join(
      os.tmpdir(),
      `autofix-minimal-test-${Date.now()}.md`,
    );
    ...
    const minimalFixes = (results[minimalTempFilePath] || []).filter(
      (v) =>
        v.ruleNames.includes("backtick-code-elements") ||
        v.ruleNames.includes("BCE001"),
    );
