...
/**
 * Test suite for autofix functionality of the sentence-case-heading rule.
 * Ensures that the autofix produces the expected output for a fixture with sentence case violations.
 */
describe("sentence-case-heading auto-fix functionality", () => {
  ...
  test("applies auto-fixes correctly", async () => {
    ...
    const fixes = (results[tempFilePath] || []).filter(
      (v) =>
  ...
  test("identifies violations in fixture", async () => {
    ...
    const violations = (results[tempFilePath] || []).filter(
      (v) =>
        v.ruleNames.includes("sentence-case-heading") ||
        v.ruleNames.includes("SC001"),
    );
