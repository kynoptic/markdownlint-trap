/**
 * @performance
 * Performance benchmarks for shared code-block and inline-code detection utilities.
 * Correctness of these functions is covered in tests/integration/shared-utils-detection.test.js;
 * this file only guards their execution time and memory profile, so it runs under the
 * isolated single-worker performance config (npm run test:performance) where wall-clock
...
describe('Shared utility performance benchmarks', () => {
  ...
  describe('Inline Code Detection Performance', () => {
    test('handles lines with many backticks efficiently', () => {
      ...
      // Warm-up run to stabilize JIT compilation before the measured pass
      for (let pos = 0; pos < complexLine.length; pos += 10) {
        isInInlineCode(complexLine, pos);
      }
