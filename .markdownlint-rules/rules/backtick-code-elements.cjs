...
var _sharedHeuristics = require("./shared-heuristics.cjs");
// @ts-check
...
const sentenceStarters = new Set(['The', 'A', 'An', 'This', 'That', 'These', 'Those', 'It', 'They', 'We', 'You', 'He', 'She', 'New', 'Go', 'Then', 'Next', 'First', 'Second', 'Finally', 'However', 'Therefore', 'Additionally', 'Furthermore', 'In', 'On', 'At', 'For', 'With', 'From', 'To', 'Some', 'Many', 'Most', 'All', 'Few', 'Several']);
...
function backtickCodeElements(params, onError) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    /\b(?!\d+:\d+\b)(?<!\d\.)[\w.-]+:(?!\d*1\b)\d+\b/g, /\b[A-Z]+\+[A-Z]\b/g,
    ...
    for (const pattern of patterns) {
      ...
      while ((match = pattern.exec(line)) !== null) {
        ...
        if ((0, _sharedHeuristics.isDomainInProse)(fullMatch, line, start)) {
          continue;
        }
