...
function basicSentenceCaseHeadingFunction(params, onError) {
  ...
  const specialCasedTerms = {
    ..._sharedConstants.specialCasedTerms
  };
  ...
  function validate(headingText, lineNumber, sourceLine, reportFn) {
    ...
    while (firstIndex < words.length && (words[firstIndex].startsWith('__PRESERVED_') && words[firstIndex].endsWith('__') || numeric.test(words[firstIndex]))) {
      firstIndex++;
    }
  ...
  tokens.forEach(token => {
    if (token.type === 'atxHeading') {
      ...
      if (listMatch) {
        ...
        const textToValidate = originalBoldText.includes(':') ? originalBoldText.split(':')[0] : originalBoldText;
